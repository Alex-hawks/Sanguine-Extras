...
import net.minecraft.client.renderer.{BlockRendererDispatcher, GlStateManager, Tessellator, WorldRenderer}
...
object Render {
  ...
  def drawFakeBlock(state: IBlockState, min: (Double, Double, Double), w: World, v: Vector3) = {
    ...
    val model: IBakedModel = dispatcher.getModelFromBlockState(state, w, null)
